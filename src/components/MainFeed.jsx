...
const MainFeed = () => {
  ...
  const getFilteredPosts = () => {
    ...
    if (searchQuery) {
      filtered = filtered.filter(post => 
        ...
        (post.board && post.board.toLowerCase().includes(searchQuery.toLowerCase())) ||
  ...
  // 사이드바 토글 — body 클래스 추가로 레이아웃 shift 방지
  const toggleSidebar = () => {
    setShowSidebar(prev => {
      const next = !prev;
      if (typeof document !== 'undefined') {
        if (next) document.body.classList.add('with-sidebar-open');
        else document.body.classList.remove('with-sidebar-open');
      }
      return next;
    });
  };

  // 컴포넌트 언마운트시 body 클래스 정리
  useEffect(() => {
    return () => {
      if (typeof document !== 'undefined') {
  ...
  const handleTabTouchStart = (e) => {
    ...
    const handleTouchEnd = (endEvent) => {
      ...
      if (Math.abs(swipeDistance) > 50) {
        if (swipeDistance > 0) setActiveTab('following');
    ...
    const onScroll = () => {
      ...
      if (!ticking) {
        window.requestAnimationFrame(() => {
          if (currentY > lastScrollY + 10 && currentY > 60) setHeaderHidden(true);
          else if (currentY < lastScrollY - 5) setHeaderHidden(false);
    ...
    <div className="main-feed" onTouchStart={handleTouchStart}>
      ...
      <div className="fixed-header">
        <div className="header">
          ...
          <div className={`header-center ${headerHidden ? 'hidden' : ''}`}>
            <h1>홈</h1>
          </div>
              ...
              <div className="post-card" onClick={() => handlePostClick(post.id)}>
                  ...
                  <div className="post-header-right">
                    ...
                    {!post.isAnonymous && (
                      <button 
                        ...
                        onClick={(e) => { e.stopPropagation(); handleFollow(post.author); }}
                        aria-pressed={followedUsers.includes(post.author)}
                      >
                        ...
                        {post.images.map((_, imgIndex) => {
                          ...
                          return (
                            <div 
                              ...
                              onClick={(e) => { e.stopPropagation(); scrollToImage(post.id, imgIndex); }}
                            />
                          );
                        })}
                      </div>
                ...
                <div className="post-actions">
                  ...
                  <button 
                    ...
                    onClick={(e) => { e.stopPropagation(); /* 저장 기능 구현 위치 */ }}
                    >
                        ...
                        <div className="cards-scroll">
                          {slideContent?.[activeSlide]?.items?.map((item, cardIndex) => (
                            <div 
                              ...
                              onClick={() => { setSearchQuery(item.text.replace('#', '')); setShowSearch(true); }}
                            >
      ...
      <div className={`sidebar ${showSidebar ? 'open' : ''}`}>
        <div className="sidebar-header">
          <h3>메뉴</h3>
          <button className="close-sidebar" onClick={toggleSidebar} aria-label="사이드바 닫기">
            <X size={20} />
          </button>
        </div>
        <div className="sidebar-content">
          <div className="sidebar-section">
            ...
            <div className="sidebar-items">
              <button className="sidebar-item"><div className="sidebar-icon">🎭</div><span>연극동아리</span></button>
              <button className="sidebar-item"><div className="sidebar-icon">💻</div><span>프로그래밍동아리</span></button>
              <button className="sidebar-item"><div className="sidebar-icon">🎵</div><span>밴드동아리</span></button>
              <button className="sidebar-item"><div className="sidebar-icon">📚</div><span>독서동아리</span></button>
            </div>
          </div>
          <div className="sidebar-section">
            <h4>기능</h4>
            <div className="sidebar-items">
              <button className="sidebar-item"><div className="sidebar-icon">📊</div><span>통계</span></button>
              <button className="sidebar-item"><div className="sidebar-icon">⚙️</div><span>설정</span></button>
              <button className="sidebar-item"><div className="sidebar-icon">🔔</div><span>알림 설정</span></button>
              <button className="sidebar-item"><div className="sidebar-icon">❓</div><span>도움말</span></button>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* 플로팅 추가 버튼 */}
      <button className="floating-add-button" onClick={handleAddPost} aria-label="게시물 작성">
        <Plus size={24} />
      </button>
    </div>
  );
